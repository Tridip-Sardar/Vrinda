...
const diseaseAdjustments = {
  ...
  Peach__healthy: {},
  Pepper_bell_Bacterial_spot: { N: 1.2, P: 1.1, K: 1.2 },
  ...
  Tomato__Spider_mites_Two_spotted_spider_mite: { N: 1.2, P: 1.15, K: 1.1 },
...
// Fertilizer recommendation function in JavaScript
function getFertilizerRecommendation(plant, growthStage, disease = null, soilData = {}) {
  const nutrients = { ...plantDatabase[plant]?.[growthStage] };

  if (!nutrients) {
    return { error: "Invalid plant or growth stage. Please check your inputs." };
  }

  if (disease && diseaseAdjustments[disease]) {
    for (const [nutrient, factor] of Object.entries(diseaseAdjustments[disease])) {
      nutrients[nutrient] = (nutrients[nutrient] || 0) * factor;
    }
  }

  if (soilData) {
    if ((soilData.N || 0) < 10) nutrients.N *= 1.1;
    if ((soilData.P || 0) < 10) nutrients.P *= 1.1;
    if ((soilData.K || 0) < 10) nutrients.K *= 1.1;
  }
...
router.get("/", (req, res) => {
  res.render("firtilizerCalculator/firtilizer.ejs");
});

router.post("/", (req, res) => {
  const { plant, growthStage, disease, soilData } =
    req.body;
